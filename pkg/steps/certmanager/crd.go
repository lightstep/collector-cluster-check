...
func (c CrdExists) Run(ctx context.Context, deps *steps.Deps) (steps.Option, steps.Result) {
	issuerCrd, err := deps.CustomResourceClient.ApiextensionsV1().CustomResourceDefinitions().Get(ctx, crdName, metav1.GetOptions{})
	if err != nil {
		return steps.Empty, steps.NewFailureResult(err)
	}
	return steps.Empty, steps.NewSuccessfulResult(issuerCrd.Name)
}
